...
#'                                                                                                                                                                                             '#
#' Richard Iannone (2020). DiagrammeR: Graph/Network Visualization. R package version 1.0.6.1.                                                                                                 '#
...
#' General description:                                                                                                                                                                        '#
#'                                                                                                                                                                                             '#
#' Given a pedigree, its relationship structure are returned as a dot file suitable for plotting.                                                                                              '#
#'                                                                                                                                                                                             '#
#' The resulting dot file can be edited prior to rendering with Graphviz (https://www.rdocumentation.org/packages/Rgraphviz/versions/2.16.0) or DiagrammeR packages, or conversion to a        '#
#' graphics file format with the dot application (see https://graphviz.org/).                                                                                                                  '#
#'                                                                                                                                                                                             '#
#'                                                                                                                                                                                             '#
#'                                                                                                                                                                                             '#
#' Arguments                                                                                                                                                                                   '#
...
#' @param  pedigree                                                                                                                                                                            '#
#'                                                                                                                                                                                             '#
#' A data.table/dataframe/matrix where the first three columns correspond to the identifiers for the individual, mother and father, respectively. The row giving the pedigree of an            '#
#' individual must appears before any row where that individual appears as parent. Founders use 0 (zero) in the parental columns.                                                              '#
#'                                                                                                                                                                                             '#
#' @param keep                                                                                                                                                                                 '#
#'                                                                                                                                                                                             '#
#' A logical vector identifying the rows of pedigree to retain. The default is rep(TRUE, nrow(pedigree)).                                                                                      '#
#'                                                                                                                                                                                             '#
#' @param filename                                                                                                                                                                             '#
#'                                                                                                                                                                                             '#
#' A character indicating the primary name corresponding to the output file (dotfile); default filename is "ped" and the suffix ".dot" is appended to the file name (file´s extension).        '#
#'                                                                                                                                                                                             '#
#' @param form                                                                                                                                                                                 '#
#'                                                                                                                                                                                             '#
#' Logical indicating if the node´s shape should be returned according to generation (i.e., rectangle for founders, circle for one unknown parent, ellipse for progeny). The default is        '#
#' form=FALSE (i.e., all node´s shape are represented as ellipses).                                                                                                                            '#                                 
#'                                                                                                                                                                                             '#
#' @param numeric                                                                                                                                                                              '#
#'                                                                                                                                                                                             '#
#' Convert pedigree to integer values for graphing. The default is FALSE. That is, mother and father labels are used in the graph.                                                             '#
#'                                                                                                                                                                                             '#
#' @param cols                                                                                                                                                                                 '#
#'                                                                                                                                                                                             '#
#' A character vector which defines fill colours of nodes for each generation, starting from founders up to the last generation.                                                               '#
#' The default vector is c("chocolate4", "olivedrab", "orange2", "gold", "khaki1", "aquamarine", "bisque1") for the first seven generations.                                                   '#
#'                                                                                                                                                                                             '#
#' @param url                                                                                                                                                                                  '#
#'                                                                                                                                                                                             '#
#' If not "" (the default), then url is linked to the resulting graph (see Dot User’s Manual for details).                                                                                     '#
#'                                                                                                                                                                                             '#
#' @param height                                                                                                                                                                               '#
#'                                                                                                                                                                                             '#
